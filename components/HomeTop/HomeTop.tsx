...
const HomeTop: React.FC<Props> = ({
  ...
  const renderCompanies = showCompanies && companies && (
    <div className="mb-15 flex w-full justify-between md:justify-center">
      {companies.map(
        (company, index) =>
          (!smallScreen || index < 3) && (
  ...
  return (
    <section {...props} className="container pt-88.1 px-5 md:px-0 md:pt-48.5">
      {grid}
      <div className="w-83.6 mr-auto md:w-100">
        <YHeading
